...
float BMI088_ACCEL_SEN = BMI088_ACCEL_3G_SEN; // Match ACC_RANGE_3G; verify stationary norm near 9.80665 m/s^2.
...
// Keep the frequency contract consistent with the unchanged register table below.
// 下表寄存器仍为原配置；集中配置的 ODR 数值必须与之相符。
#include "LowerControllerConfig.h"
#if LC_GYRO_ODR_HZ != 1000U || LC_ACCEL_ODR_HZ != 800U
#error "Update the BMI088 register table and ODR contract together."
#endif
...
void BMI088_accel_read_over(uint8_t *rx_buf, float accel[3], float *time)
{
    ...
    accel[0] = a[0] * C01 + a[1] * C02 + a[2] * C03;
    accel[1] = a[0] * C04 + a[1] * C05 + a[2] * C06;
    accel[2] = a[0] * C07 + a[1] * C08 + a[2] * C09;
    sensor_time = (uint32_t(rx_buf[8]) << 16) | (uint32_t(rx_buf[7]) << 8) | rx_buf[6]; // 24 位，39.0625 us/计数；调度时间仍使用 TIM2。
