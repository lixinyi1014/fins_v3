...
| 通道 | 含义 | 空气中首次观察 |
|---|---|---|
| I0 | 传感器1 / TCA0，前左，旧水压刻度 | 旧代码同单位；空气中由本次上电采样偏置后通常接近0 |
| I1 | 传感器2 / TCA1，后左，旧水压刻度 | 同上 |
| I2 | 传感器3 / TCA2，后右，旧水压刻度 | 同上 |
| I3 | 传感器4 / TCA3，前右，旧水压刻度 | 同上 |
| I4 | ESKF roll 横滚，度 | 有效前为 -9999；有效后水平接近0 |
| I5 | ESKF pitch 俯仰，度 | 有效前为 -9999；有效后水平接近0 |
| I6 | ESKF yaw 偏航，度 | 有效前为 -9999；有效后相对启动航向变化 |
| I7 | ESKF 融合深度，m | 有效前为 -9999；有效后接近0 |
...
| I14 | ERROR_CODE：停止原因/总线错误/ESKF状态 | bits0..7停止原因，bits8..15累计I2C2错误，bits16..31为ESKF flags |
| I15 | SEQ：显示包序号，65535后回0 | **持续变化** |

I0..I3是旧工程直接显示的四路压力值；水中倾斜时本来就不相等。
...
进一步验证压力编号：开机归零后，让压力传感器逐个进入浅水，观察I0..I3哪一路增大。此操作无需ON，不必把未验漏的舱体放进水中；不要吹、挤敏感膜片代替水头测试。
...
正常停止：OFF → 控制板 → IIC → PCA → 八路1550us → 电调停止，舵机保留请求。
...
V3.3设计p7：压力顺序改成前左、后左、后右、前右；推进器、舵机通道核对一致。p8：SWD/UART/IIC线序。当前实物死区为1510..1610us。
停止仍1550us；新闭环正向补60us、反向补40us；TES仍代表原始PWM。约0.59V缺完整测量条件，不能拿来设供电或替代PWM脉宽。
