...
def test_arm_depth_interlock() -> None:
    """入水互锁：岸上按 Start 只记意图，水压计确认入水才真正发 ON。

    ON 之后固件立刻让四路垂直推进器进入深度闭环，桨会转；
    空气中空转既无水冷也无负载，所以这一层拦在上位机。
    """
    print("入水互锁")
    from link import SerialLink
    from joystick_control import Controller

    def vofa(depth: float, seq: int) -> str:
        return ("vofa:1004.10,1004.20,1004.30,1004.40,0.50,-1.00,45.00,"
                f"{depth:.3f},1,1,1,15,0,15,0,{seq}")
    ...
    try:
        ...
        link.open()
        fake = created[0]
        controller = Controller(link, arm_depth_m=0.15, log=lambda _: None)

        now = time.monotonic()
        link.parser.feed_line(vofa(0.000, 1), now)
        controller.toggle_arm(now)
        check(fake.count(Command.ARM) == 0, "岸上按 Start 不发 ON")
        check(controller.arm_pending is True, "进入待入水状态")
        check(controller.arm_intent is False, "待入水时解锁意图仍为假")

        now = time.monotonic()
        link.parser.feed_line(vofa(0.080, 2), now)
        controller._check_arm_interlock(now)
        check(fake.count(Command.ARM) == 0, "深度不够阈值时仍不发 ON")
        check(controller.arm_pending is True, "继续等待")

        now = time.monotonic()
        link.parser.feed_line(vofa(0.200, 3), now)
        controller._check_arm_interlock(now)
        check(fake.count(Command.ARM) == 1, "确认入水后自动发 ON")
        check(controller.arm_intent is True, "解锁意图置位")
        check(controller.arm_pending is False, "待入水状态已清除")
        check(abs(controller.target_depth_m - 0.200) < 1e-6,
              "目标深度用入水那一刻的实测值初始化")

        # 待入水期间再按一次 Start 应当取消，而不是排队等着
        controller.disarm("测试")
        now = time.monotonic()
        link.parser.feed_line(vofa(0.000, 4), now)
        controller.toggle_arm(now)
        check(controller.arm_pending is True, "重新进入待入水")
        controller.toggle_arm(now)
        check(controller.arm_pending is False, "再按一次 Start 取消待入水")

        # 急停必须能打断待入水，否则人走开后它还会自己解锁
        controller.toggle_arm(now)
        check(controller.arm_pending is True, "第三次进入待入水")
        controller.emergency_stop("测试")
        check(controller.arm_pending is False, "急停清除待入水")

        # 关闭互锁时保持原行为
        link2 = SerialLink("COM_FAKE", log=lambda _: None)
        link2.open()
        fake2 = created[-1]
        c2 = Controller(link2, arm_depth_m=0.0, log=lambda _: None)
        link2.parser.feed_line(vofa(0.000, 1), time.monotonic())
        c2.toggle_arm(time.monotonic())
        check(fake2.count(Command.ARM) == 1, "互锁关闭时岸上也直接发 ON")

        link.close(send_disarm=False)
        link2.close(send_disarm=False)
    finally:
        restore()
