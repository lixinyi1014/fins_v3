...
def load_mapping(path: str | None) -> dict:
    ...
    找不到文件时直接报错退出，绝不悄悄退回默认映射：
    默认映射是按 Xbox 布局猜的，用错手柄会让舵机一开机就跑到底。
    """
    ...
    if not candidate.is_file():
        beside = Path(__file__).with_name(candidate.name)
        if beside.is_file():
            candidate = beside
        else:
            raise SystemExit(
                f"找不到映射文件 {path}。\n"
                f"先运行 `python pad_probe.py` 做一遍标定，"
                f"它会把 my_pad.json 写到 {Path(__file__).parent}。")
...
class GamepadReader:
    def __init__(self, mapping: dict, *, log=print) -> None:
        ...
        self._disabled_axes: set[int] = set()
    ...
    def _acquire(self) -> None:
        ...
        self._blacklist_resting_axes(joystick)

    def _blacklist_resting_axes(self, joystick) -> None:
        """静止时就远离中位的轴（扳机）一律禁用。

        没有这道保护时，映射一旦把扳机填成摇杆，_update_servos 会拿到
        一个恒定满偏值，舵机在程序启动后一两秒内就顶到行程尽头 ——
        而舵机不受固件停止锁存保护，这是真动作，不是显示问题。
        """
        self._disabled_axes = set()
        for index in range(joystick.get_numaxes()):
            value = joystick.get_axis(index)
            if abs(value) > 0.5:
                self._disabled_axes.add(index)
        if not self._disabled_axes:
            return
        used = {name: i for name, i in self.mapping["axes"].items()
                if i in self._disabled_axes}
        self._log(f"[手柄] 轴 {sorted(self._disabled_axes)} 静止时读数就远离中位"
                  "（多半是扳机），已禁用")
        for name, index in used.items():
            self._log(f"[手柄] 警告：映射把 {name} 指到了轴 {index}，"
                      "该通道将恒为 0，请重新运行 pad_probe.py 标定")
    ...
    def _axis(self, name: str) -> float:
        ...
        if index >= self._joystick.get_numaxes():
            return 0.0
        if index in self._disabled_axes:
            return 0.0   # 见 _blacklist_resting_axes
